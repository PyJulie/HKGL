...
class ResampleLoss(nn.Module):
    def __init__(self,
                 ...
                 num_list = None, neg_num_list = None):
        ...
        if self.use_sigmoid:
            ...
            self.cls_criterion = F.binary_cross_entropy
        ...
        self.class_freq = torch.from_numpy(num_list).float().cuda()
        self.neg_class_freq = torch.from_numpy(neg_num_list).float().cuda()
    ...
    def forward(self,
        ...
        if self.focal:
            logpt = - self.cls_criterion(
                cls_score.clone(), label, weights=None, reduction='none',
